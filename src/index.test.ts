...
test("Should publish is-enabled event when isEnabled is called", (done) => {
    fetchMock.mockResponseOnce(JSON.stringify(data));
    const config: IConfig = {
        url: "http://localhost/test",
        clientKey: "12",
    ...
    client.on(EVENTS.IS_ENABLED, (event: any) => {
...
test("Should publish get-variant event when getVariant is called", (done) => {
    fetchMock.mockResponseOnce(JSON.stringify(data));
    const config: IConfig = {
        url: "http://localhost/test",
        clientKey: "12",
    ...
    client.start();
    client.on(EVENTS.READY, () => {
        const isEnabled = client.getVariant("simpleToggle");
    });

    client.on(EVENTS.GET_VARIANT, (event: any) => {
