...
- **ready** - emitted after the SDK has successfully started and performed the initial fetch towards the Unleash Proxy. When bootstrapping client can emit this event twice - once when bootstrapped flags are loaded, and once on first connection to Unleash.
